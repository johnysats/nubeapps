...
from kxsim import camera, device, remote, screen
...
class Handler(BaseHTTPRequestHandler):
    ...
    def log_message(self, fmt, *args):
        logger.debug("kxsim-web: " + fmt, *args)
    ...
    def do_POST(self):
        ...
        if path == "/api/touch":
            ...
            camera.source.set_noise((form.get("noise") or [""])[0] == "1")
            self._send(json.dumps(self._state()).encode(), "application/json")
...
def start(port: int):
    ...
    logger.info("kxsim: interfaz web escuchando en :%s", port)
    return server
