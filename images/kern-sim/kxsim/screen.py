...
A diferencia de krux y seedsigner, el firmware de Kern es un binario nativo que dibuja con
SDL: no hay un PIL.Image al que engancharse desde adentro. Corre contra un Xvfb y la pantalla
se saca del framebuffer X, como en ccq1 -- pero sin VNC, porque una conexion infinita se
atasca en los reverse proxies y deja los clicks encolados detras.
"""
import os
import re
import subprocess
...
# Techo de refresco. El simulador redibuja a 30 fps aunque no cambie nada; la comparacion
# contra el frame anterior evita publicar (y comprimir) lo que el usuario no va a ver.
MAX_FPS = 12

DISPLAY = os.environ.get("KXSIM_DISPLAY", ":99")
WINDOW_NAME = "Kern Simulator"
...
def wait_for_window(timeout=30.0):
    ...
    while time.monotonic() < deadline:
        ...
        if match:
            ...
            return geometry
        time.sleep(0.3)
    raise RuntimeError(f"kxsim: la ventana '{WINDOW_NAME}' no aparecio en {timeout:.0f}s")
...
def run():
    ...
    while True:
        ...
        try:
            image = grab()
            # Comparar los bytes es mas barato que codificar un JPEG que nadie pidio: el
            # simulador dibuja continuamente aunque la pantalla no cambie.
            raw = image.tobytes()
            if raw != previous:
                previous = raw
                framebuffer.put(image)
        except OSError as error:  # el servidor X se cae o el simulador se esta reiniciando
            print("kxsim: no pude capturar la pantalla:", error)
            time.sleep(1)
