...
El frame se sirve como JPEG suelto: una conexion infinita tipo MJPEG se atasca en los reverse
...
from ksemu import camera, device, remote, screen
...
class Handler(BaseHTTPRequestHandler):
    ...
    def log_message(self, fmt, *args):
        logger.debug("ksemu-web: " + fmt, *args)
    ...
    def do_POST(self):
        ...
        if path == "/api/touch":
            ...
            try:
                ...
                self._send(b'{"error":"coordenadas invalidas"}', "application/json", status=400)
                return
            remote.touch(x, y, action)
    ...
    def _state(self):
        ...
        return {
            ...
            # Cuantos QR quedaron listos y, si el archivo no servia, por que.
            "parts": camera.source.parts,
            "error": camera.source.error,
        }
...
def start(port: int):
    ...
    logger.info("ksemu: interfaz web escuchando en :%s", port)
    return server
